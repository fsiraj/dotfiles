...
local function generate_nvim_overrides(p)
   ...
   return {
      ...
      --[[ Pmenu       ]] PmenuSel = { bg = p.subtext },
      --[[ Other       ]] CursorLineNr = { fg = p.accent }, Folded = { bg = p.base }, NonText = { link = 'LineNr' }, WinSeparator = { fg = p.black },
      -- Plugins
      --[[ Dap         ]] DapBreak = { fg = p.red }, DapStop = { fg = p.yellow },
      --[[ Flash       ]] FlashLabel = { fg = p.crust, bg = p.accent }, FlashPrompt = { link = 'Normal' },
      --[[ Foldtext    ]] FoldtextLines = { fg = p.cyan },
      ...
      --[[ Mini        ]] MiniIndentscopeSymbol  = { fg = p.black },
      ...
      --[[ Treesitter  ]] TreesitterContext = { bg = p.base }, TreesitterContextSeparator = { fg = p.black, bg = p.base },
   }
end
...
local core_plugins = {
   ...
   --Sidekick
   {
      ...
      config = function()
         ...
         --stylua: ignore start
         keymap('n', '<leader>aa', function() cli.toggle({ filter = { cwd = true } }) end, { desc = 'Attach CLI' })
         keymap('n', '<leader>as', function() cli.select({ filter = { installed = true } }) end, { desc = 'Select CLI' })
         keymap('n', '<leader>ac', function() cli.select({ filter = { installed = true }, continue = true }) end, { desc = 'Continue CLI' })
         ...
         snacks.setup({
            ...
            toggle = {
               notify = function(state, opts)
                  vim.notify(state and 'Enabled' or 'Disabled', vim.log.levels.INFO, { title = opts.name })
               end,
            },
            dashboard = {
               sections = {
   ...
   --Copilot
   {
      ...
      cmd = 'Copilot',
      keys = { { '<Leader>Tc', desc = 'Enable Copilot' } },
      ...
      config = function()
         ...
         require('snacks')
            .toggle({
               name = 'Copilot',
               get = function() return vim.g.copilot_enabled end,
               set = function(state)
...
local language_plugins = {
   ...
   --Blink
   {
      ...
      opts = {
         ...
         sources = {
            ...
            per_filetype = { ['dap-repl'] = { 'dap', 'path' } },
            providers = {
   ...
   --Conform
   {
      ...
      config = function()
         require('conform').setup({
            ...
            formatters = { shfmt = { append_args = { '-ci' } } },
         })
   ...
   --RenderMarkdown
   {
      ...
      ft = { 'markdown' },
      keys = { { '<Leader>Tm', desc = 'Disable Markdown Rendering' } },
      dependencies = {
         'nvim-treesitter/nvim-treesitter',
         'nvim-tree/nvim-web-devicons',
      },
      ...
      config = function(_, opts)
         ...
         markdown.setup(opts)
         require('snacks')
            .toggle({ name = 'Markdown Rendering', get = markdown.get, set = markdown.set })
            :map('<Leader>Tm')
      end,
   },
...
require('lazy').setup(specs, {
   headless = { task = false, process = false },

...
local function get_palette(base)
   base = base or require('osc-colors').get_palette()
   --stylua: ignore
   return {
...
local function generate_nvim_overrides(p)
   ...
   return {
      ...
      --[[ Pmenu       ]] Pmenu= { link = 'NormalFloat' }, PmenuSel = { bg = p.black },
      ...
      --[[ Visual      ]] Visual= { bg = p.black },
      --[[ Window      ]] WinSeparator= { fg = p.crust, bg = p.base },
      --[[ Semantics   ]] Keyword = {fg = p.accent },
      -- Plugins
...
local function generate_lualine_theme()
   local p = get_palette()
   return {
      normal = {
      ...
      command = {
         a = { bg = p.green, fg = p.crust, gui = 'bold' },
      },
      terminal = {
         a = { bg = p.green, fg = p.crust, gui = 'bold' },
      },
...
local function reload_nvim_plugins()
   ...
   local plugins_to_reload = { ['fzf-lua'] = 'fzf-lua', ['tiny-glimmer.nvim'] = 'tiny-glimmer' }
   ...
   if package.loaded['lualine'] then
      ...
      config.options.theme = generate_lualine_theme()
...
autocmd('ColorScheme', { callback = reload_nvim_plugins })
...
local ui_plugins = {
   --OscColors
   {
      'fsiraj/osc-colors.nvim',
      dev = false,
      priority = 1000,
      lazy = false,
      opts = {
         highlights = { overrides = function(base) return generate_nvim_overrides(get_palette(base)) end },
